<template>
  <div class="goodsinfo-container">
    ...
    <!-- 商品购买区域 -->
		<div class="mui-card">
          ...
          <p>
            ...
            <!-- 1.经过分析发现： 按钮属于goodsinfo页面，数字 属于numberbox 组件 -->
            <!-- 2.由于涉及到了父子组件的嵌套了，所以，无法直接在goodsinfo页面中获取到选中的商品数量值 -->
            <!-- 3.涉及到了子组件向父组件传值了（事件调用机制） -->
            <!-- 4.事件调用的本质：父向子传递方法，子调用这个方法，同时把 数据当参数，传递给这个方法 -->
          </p>
				</div>
			</div>
		</div>
    ...
    <div class="mui-card">
        ...
        <p>上货时间: {{ goodsinfo.add_time | dataFormat }} </p>
				</div>
			</div>
...
export default {
  ...
  methods: {
    ...
    addToShopCar(){
      ...
      //拼接出一个，要保存到store 中car数组里的商品信息对象
      var goodsinfo = { 
      id: this.id, 
    ...
    enter(el,done){
      ...
      //3. 因此，我们经过分析，得到结论；不能把位置的横纵坐标直接写死了，而是应该 根据不同情况，动态计算这个坐标值； 
      //4. 经过分析，得到解题思路：先得到徽标的横纵坐标，在得到小球的横纵坐标，然后让x,y求差，得到的结果，就是横纵坐标要位移的距离
    ...
    getSelectedCount(count){
      //当子组件把选中的数量传递给父组件的时候，把选中的值保存到data上
        ...
        console.log('父组件拿到的数量值为: ' + count + "次数")
    }
  },
  components: {
    swiper,
